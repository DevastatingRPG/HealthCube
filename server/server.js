...
app.get("/download", (req, res) => {
    const { form, uname } = req.query;
    const file = `files/fforms/${form}/${uname}.txt`;
    res.download(file);
});
...
app.post("/login", (req, res) => {
    ...
    connection.query(query, [UID, password], (err, results) => {
        ...
        if (results.length > 0) {
            ...
            const token = jwt.sign({ UID }, JWT_SECRET_KEY, {
                expiresIn: "30d",
            });
