...
//       ];
//     // Fetch leaderboard data from AsyncStorage or your backend
...
useEffect(() => {
    ...
    const rankedData = sampleData.map((item, index) => ({
        ...item,
        rank: index + 1,
      }));
    AsyncStorage.setItem('leaderboardData', JSON.stringify(rankedData))
      .then(() => {
        ...
        const user = rankedData.find((item) => item.username === Mainuser);
